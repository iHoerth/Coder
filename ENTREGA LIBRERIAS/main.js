...
const renderizarAlumno = (alum,showAlert) => {
  //showAlert espera un boolean, true muestra el swal, false no. Lo hice asi para que no tire alerta cuando renderiza el localStorage al tirar f5.
  ...
  if(showAlert){
    swal({
      ...
      text: `Nombre ${alum.nombre} ${alum.apellido} \nDNI ${alum.dni} \nCURSO ${alum.curso}`,
      icon: 'success',
      confirm: 'Ok',
    })
  }
...
form.addEventListener('submit', (e) => {
  ...
  if(!validacionDatos(e)){
    ...
    renderizarAlumno(nuevoAlum,true); // mostrarle al usuario los cambios
    limpiarFormulario();
  }
})
